...
function PresaleModule({
  ...
  const handleContribute = async () => {
    ...
    if (!telegramUser) {
      setStatusMessage(
        "Telegram user data not available. Please restart the bot."
      );
      return;
    }
    if (
      isNaN(parseFloat(amountToContribute)) ||
      parseFloat(amountToContribute) <= 0
    ) {
      setStatusMessage("Please enter a valid amount.");
      return;
    }
    if (
      parseFloat(amountToContribute) < metrics.minBuy ||
      parseFloat(amountToContribute) > metrics.maxBuy
    ) {
      setStatusMessage(
        `Amount must be between ${metrics.minBuy} and ${metrics.maxBuy}.`
      );
      return;
    }
    if (!authToken) {
      setStatusMessage("Authentication failed. Please refresh the page.");
      return;
    }
  ...
  // Effect to record contribution on the backend after transaction confirmation
  useEffect(() => {
    if (isConfirmed && receipt) {
      setStatusMessage(
        `Success! Transaction confirmed. Your TX Hash is: ${receipt.transactionHash}`
      );
      setLoading(false);
      onContributionSuccess();
      ...
      axios
        .post(
          `${API_BASE_URL}/record-contribution`,
          {
          ...
          {
            headers: {
              'Authorization': `Bearer ${authToken}`, // CORRECTED: Use the prop for the header
            },
          }
        )
        .catch((backendError) => {
          console.error(
            "Failed to record contribution on backend:",
            backendError
          );
          setStatusMessage(
            `Transaction confirmed, but failed to record on server: ${backendError.message}`
          );
        });
    }
  }, [
    ...
    onContributionSuccess,
    API_BASE_URL,
    authToken, // ADDED: Include authToken in dependencies
  ]);
  ...
  if (!metrics) {
    return (
      <div className="text-center text-gray-400">Loading presale data...</div>
    );
  }

  const calculatedHDC = amountToContribute
    ? (parseFloat(amountToContribute) / metrics.tokenPrice).toFixed(2)
    : "0.00";
  ...
  return (
    <div className="space-y-6">
      ...
      {userTokens >= 0 && (
    <div className="text-center text-lg font-medium text-white bg-blue-800 bg-opacity-30 p-4 rounded-lg">
      Your Purchased Tokens:{" "}
      <span className="text-green-400 font-bold">
        {userTokens.toLocaleString(undefined, { maximumFractionDigits: 2 })}{" "}
        $HDC
      </span>
    </div>
)}
      ...
      <div className="grid grid-cols-2 gap-4 text-center bg-gray-700 p-4 rounded-lg shadow-inner">
        <div>
          ...
          <p className="text-xl font-bold text-green-400">
            {metrics.totalRaised.toFixed(2)} BNB
          </p>
        </div>
        <div>
          <p className="text-sm text-gray-400">Soft Cap / Hard Cap</p>
          <p className="text-xl font-bold">
            {metrics.softCap} / {metrics.hardCap} BNB
          </p>
        </div>
        <div>
          <p className="text-sm text-gray-400">Current Price</p>
          <p className="text-xl font-bold text-yellow-400">
            1 BNB = {(1 / metrics.tokenPrice).toFixed(0)} $HDC
          </p>
        </div>
        <div>
          <p className="text-sm text-gray-400">Min/Max Buy</p>
          <p className="text-xl font-bold">
            {metrics.minBuy} / {metrics.maxBuy} BNB
          </p>
        </div>
      </div>
      ...
      <div className="bg-gray-700 p-4 rounded-lg shadow-inner space-y-4">
        ...
        <div className="flex flex-col">
          <label htmlFor="amount" className="text-gray-300 text-sm mb-1">
            Amount (BNB)
          </label>
          <input
            type="number"
        ...
        <p className="text-sm text-gray-400">
          ...
          <span className="font-bold text-green-400">{calculatedHDC} $HDC</span>
        </p>
